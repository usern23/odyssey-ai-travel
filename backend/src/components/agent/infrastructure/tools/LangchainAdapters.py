...
from src.components.agent.infrastructure.tools.auxiliary.AviasalesTool import AviasalesTool
from src.components.agent.infrastructure.tools.auxiliary.AmadeusClient import AmadeusClient, get_amadeus_client
from src.components.agent.infrastructure.tools.auxiliary.YouTubeSearchTool import YouTubeSearchTool
from src.components.agent.infrastructure.tools.strategic.DestinationSuggester import DestinationSuggester
from src.components.agent.infrastructure.tools.strategic.TripDataCollector import TripDataCollector
...
class DestinationSuggesterInput(BaseModel):
    ...
    budget: Optional[str] = Field(
        default=None,
        description="Budget preference: 'economy', 'comfort', or 'unlimited'")
...
class YouTubeSearchInput(BaseModel):
    query: str = Field(
        description=(
            'Поисковый запрос. Например: "Прага travel guide", '
            '"что посмотреть в Барселоне за 3 дня", "Tokyo street food vlog".'
        )
    )
    max_results: int = Field(
        default=5,
        description='Сколько видео вернуть (1-10, по умолчанию 5).',
    )
    language: Optional[str] = Field(
        default=None,
        description=(
            "ISO 639-1 код языка для приоритета (например 'ru', 'en'). "
            'Опционально.'
        ),
    )


class YouTubeSearchLangTool(BaseTool):
    name: str = 'youtube_search'
    description: str = (
        'Найти YouTube-видео по теме поездки или направлению.\n\n'
        'Используй когда:\n'
        '- Пользователь спрашивает "покажи видео про X"\n'
        '- Пользователь хочет travel guide / vlog / обзор города\n'
        '- В ответ к плану полезно приложить 2-3 видео по конкретному городу\n\n'
        'Возвращает JSON-список с url, title, channel, published_at, thumbnail.\n'
        'Используй короткие конкретные запросы. По умолчанию max_results=5.'
    )
    args_schema: Type[BaseModel] = YouTubeSearchInput
    _tool: YouTubeSearchTool = None  # type: ignore[assignment]

    def __init__(self, tool: YouTubeSearchTool, **kwargs: Any):
        super().__init__(**kwargs)
        self._tool = tool
    ...
    async def _arun(
        ...
        import json
        result = await self._tool.search_videos(
            query=query, max_results=max_results, language=language)
        if not result.get('success'):
            return (
                f"Не удалось получить видео: {result.get('error', 'unknown')}"
            )
        videos = result.get('videos') or []
        if not videos:
            return 'По запросу видео не найдены.'

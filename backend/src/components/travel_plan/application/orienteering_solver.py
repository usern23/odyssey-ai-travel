...
class OrienteeringSolver:
    ...
    def solve(
        ...
        self._compute_qualities(candidates, user_preferences or {})
        ...
        logger.debug(
            f'OP solved: {iteration} iters, F*={best.objective:.4f}, '
            f'visited={sum(len(r) for r in best.routes)}/{len(candidates)}'
        )
    ...
    def _compute_qualities(
        ...
        for p in candidates:
            ...
            p.quality = cfg.beta_rating * r_norm + cfg.beta_preference * u_ki
    ...
    def _greedy_initial(
        ...
        """
        Жадная конструктивная эвристика:
        сортируем кандидатов по отношению q_i / v_i (полезность на единицу времени),
        последовательно вставляем в день с наименьшей нагрузкой,
        проверяя ограничения T_max и B_max.
        """
        ...
        for place in sorted_candidates:
            ...
            for d in range(num_days):
                # Оценка добавочного времени:
                # t(last → place) + v_place + t(place → depot) − t(last → depot)
